...
*
*******************************************************************************/
void ClkDrvr_1_Start(void) 
{
    /* Set the bit to enable the clock. */
    ClkDrvr_1_CLKEN |= ClkDrvr_1_CLKEN_MASK;
	ClkDrvr_1_CLKSTBY |= ClkDrvr_1_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: ClkDrvr_1_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ClkDrvr_1_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    ClkDrvr_1_CLKEN &= (uint8)(~ClkDrvr_1_CLKEN_MASK);
	ClkDrvr_1_CLKSTBY &= (uint8)(~ClkDrvr_1_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ClkDrvr_1_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ClkDrvr_1_StopBlock(void) 
{
    if ((ClkDrvr_1_CLKEN & ClkDrvr_1_CLKEN_MASK) != 0u)
    {
...
#if defined(ClkDrvr_1__CFG3)
        CLK_DIST_AMASK = ClkDrvr_1_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ClkDrvr_1_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ClkDrvr_1__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ClkDrvr_1_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ClkDrvr_1_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ClkDrvr_1_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ClkDrvr_1_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        ClkDrvr_1_CLKSTBY &= (uint8)(~ClkDrvr_1_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: ClkDrvr_1_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ClkDrvr_1_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = ClkDrvr_1_GetSourceRegister();
    uint16 oldDivider = ClkDrvr_1_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ClkDrvr_1_CLKEN & ClkDrvr_1_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ClkDrvr_1_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: ClkDrvr_1_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 ClkDrvr_1_GetDividerRegister(void) 
{
    return CY_GET_REG16(ClkDrvr_1_DIV_PTR);
}


/*******************************************************************************
* Function Name: ClkDrvr_1_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ClkDrvr_1_SetModeRegister(uint8 modeBitMask) 
{
    ClkDrvr_1_MOD_SRC |= modeBitMask & (uint8)ClkDrvr_1_MODE_MASK;
}


/*******************************************************************************
* Function Name: ClkDrvr_1_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ClkDrvr_1_ClearModeRegister(uint8 modeBitMask) 
{
    ClkDrvr_1_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ClkDrvr_1_MODE_MASK));
}


/*******************************************************************************
* Function Name: ClkDrvr_1_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ClkDrvr_1_GetModeRegister(void) 
{
    return ClkDrvr_1_MOD_SRC & (uint8)(ClkDrvr_1_MODE_MASK);
}


/*******************************************************************************
* Function Name: ClkDrvr_1_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ClkDrvr_1_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = ClkDrvr_1_GetDividerRegister();
    uint8 oldSrc = ClkDrvr_1_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ClkDrvr_1_MOD_SRC |= CYCLK_SSS;
        ClkDrvr_1_MOD_SRC =
            (ClkDrvr_1_MOD_SRC & (uint8)(~ClkDrvr_1_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ClkDrvr_1_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ClkDrvr_1_GetSourceRegister(void) 
{
    return ClkDrvr_1_MOD_SRC & ClkDrvr_1_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ClkDrvr_1_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ClkDrvr_1_SetPhaseRegister(uint8 clkPhase) 
{
    ClkDrvr_1_PHASE = clkPhase & ClkDrvr_1_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ClkDrvr_1_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ClkDrvr_1_GetPhaseRegister(void) 
{
    return ClkDrvr_1_PHASE & ClkDrvr_1_PHASE_MASK;
}
